export const partenaires = [
  // Ministéres
  "Ministère de l'Economie et des Finances",
  "Ministère de l'Industrie et du Commerce",
  "Ministère de l'Éducation Nationale, du Préscolaire et des Sports",
  "Ministère de l'Inclusion Économique, de la Petite Entreprise, de l’Emploi et des Compétences",
  "Ministère de la Transition Numérique et de la Réforme de l'Administration",
  "Ministère de la Transition Énergétique et du Développement Durable",
  "Ministère de l'interieur",
  "Ministère de l'équipement, du transport, de la logistique et de l'eau",
  "Ministère de l'agriculture, de la pêche maritime, du développement rural et des eaux et forêts",
  "Ministère de la jeunesse, de la culture et de la communication",
  "Ministère de l'aménagement du territoire national, de l'urbanisme, de l'habitat et de la politique de la ville",
  "Ministère délégué auprès du Ministre de l'Économie et des Finances chargé du Budget",



  //Etablissements publics et offices
  "Centre Régional d'Investissement Souss-Massa",
  "CGEM Souss-Massa",
  "Agence Urbaine d'Agadir",
  "Direction Régionale de l'Agriculture",
  "Direction Régionale de l'Équipement et du Transport",
  "Direction Régionale de la Santé",
  "Académie Régionale d'Éducation et de Formation",
  "Office Régional de Mise en Valeur Agricole",
  "Chambre de Commerce, d'Industrie et de Services",
  "Chambre d'Agriculture",
  "Chambre d'Artisanat",
  "Chambre des Pêches Maritimes",
  "ONMT (Office National Marocain du Tourisme)",
  "Agence Nationale de Promotion de l'Emploi et des Compétences (ANAPEC)",
  "Agence Marocaine pour le Développement des Activités Logistiques (AMDL)",
  "Agence Nationale pour le Développement des Zones Oasiennes et de l'Arganier (ANDZOA)",



  // Collectivités territoriales
  "Wilaya de la Région Souss Massa",
  "Région Souss Massa",
  "Préfecture de Agadir Ida Outanane",
  "Préfecture de Inezgane Ait Melloul",
  "Province de Tiznit",
  "Province de Taroudannt",
  "Province de Tata",
  "Province de Chtouka Ait Baha",
  "Conseil Provincial de Tata",
  "Conseil Provincial de Tiznit",
  "Conseil Provincial de Taroudannt",
  "Conseil Provincial de Chtouka Ait Baha",

  // Établissements d'enseignement et de formation
  "Université Ibn Zohr",
  "ENCG Agadir",
  "ENSA Agadir",
  "EST Agadir",
  "OFPPT",

  // Organismes internationaux
  "Union Européenne",
  "Banque Mondiale",
  "Banque Africaine de Développement",
  "PNUD",
  "GIZ",
  "AFD",
  "USAID",

  // Secteur privé et associations
  "Société MedZ",
  "Association des Industriels d'Agadir",
  "Cluster Digital Souss-Massa",
  "Technopark Agadir",
  "Haliopolis",
  "Agrotech Souss-Massa",
  "Association des Exportateurs d'Agrumes",
  "Association des Producteurs de Primeurs",
  "Fédération du Tourisme",
  "Association des Investisseurs de la Zone Industrielle",
  "CDG",
  "Banque Populaire",
  "Attijariwafa Bank",
  "BMCE Bank",
  "CIH Bank",
  "Crédit Agricole du Maroc",
  "CCG"
] as const;

export type Partenaire = typeof partenaires[number];